...
const SUPPORTED_VERIFICATION_PROFILES = Object.freeze(['backend', 'frontend', 'full', 'docs'])
...
export function resolveUserConfig(userConfig) {
  ...
  const missing = SUPPORTED_VERIFICATION_PROFILES.filter((name) => !profileNames.includes(name))
  if (missing.length > 0) {
    throw new Error(`User config must define verification profiles: ${missing.join(', ')}`)
  }
  const unsupported = profileNames.filter((name) => !SUPPORTED_VERIFICATION_PROFILES.includes(name))
  if (unsupported.length > 0) {
    throw new Error(`Unsupported verification profile(s): ${unsupported.join(', ')}`)
  }

  for (const gate of ['checkpoint', 'final']) {
    const commands = verification[gate]
    if (!isCommandList(commands)) {
      throw new Error(`User config must define verification.${gate} commands`)
    }
  }
