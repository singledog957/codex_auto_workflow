...
export function renderReport(state) {
  ...
  const reviewDetail = state.finalReview?.mode && state.finalReview?.round
    ? ` (${state.finalReview.mode} round ${state.finalReview.round})`
    : ''
  const lines = [
    ...
    `- Independent review: ${state.finalReview?.status ?? 'not run'}${reviewDetail}`,
  ]
  ...
  if (state.finalReview?.advisories?.length) {
    lines.push('', '### Review advisories', '')
    lines.push(...state.finalReview.advisories.map((advisory) => `- ${advisory}`))
  }
  lines.push('')
  return lines.join('\n')
}
