...
const VERIFICATION_PROFILES = new Set(['backend', 'frontend', 'full', 'docs'])
...
export function effectiveTaskType(task) {
  ...
  return /^checkpoint\b/i.test(task.title ?? '') ? 'checkpoint' : 'implementation'
}

function validateTaskDefinition(task, label = task.id ?? 'Task') {
  ...
  if (task.maxFiles !== undefined && (!Number.isInteger(task.maxFiles) || task.maxFiles < 1 || task.maxFiles > 5)) {
    throw new Error(`${label} maxFiles must be an integer from 1 to 5`)
  }
  if (!VERIFICATION_PROFILES.has(task.verificationProfile)) {
...
export function validateCheckpointReview(review) {
  ...
  review.findings.forEach((finding, index) => validateTaskDefinition(finding, `Checkpoint finding ${index + 1}`))
  return review
}
...
export function addCheckpointFindings(
  ...
  validateCheckpointReview(review)
